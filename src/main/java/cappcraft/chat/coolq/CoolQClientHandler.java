...
public class CoolQClientHandler extends SimpleChannelInboundHandler<WebSocketFrame> {
    ...
    private static final AttributeKey<String> Server_Role = AttributeKey.valueOf("Server_Role");
    ...
    public void onChatEvent(ServerChatEvent chat){
        for (Channel ch :
                proxy.channels) {
            if(ch.hasAttr(Server_Role) && ch.attr(Server_Role).get().equals("Universal")){
